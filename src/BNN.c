              ...
              int *total_iteration, int *popN)
{
  ...
  int stepscale = 500;
  int WARM=1; 
  static double *refden, tau=0.6, rho=1.0, delta;
  static int grid, accept_mut, total_mut;
  static double hightem=1.0, lowtem=1.0, seltem=1.0;     
  static int dim, connection_threshold=25;
  GetRNGstate();
  //begin "check.c"
  int check(W)
    int *W;
  {
      int i, j,s,L, **node, Okay;
      
      for(L=0, i=1; i<=dim; i++) L+=W[i];
      if(L==0) Okay=0;
      else{
        ...
        while(Okay==1 && i<=(*hd1)){
          ...
          if((node[i][1]==0 && node[i][2]>0)||(node[i][1]>0 && node[i][2]==0)) Okay=0;
          i++;
        }
  ...
  int add_connection_prior(W,fz,tem,region,hist)
    double *fz, tem,**hist;
  int *W,*region;
  {
  ...
  int del_connection_prior(W,fz,tem,region,hist)
    double *fz, tem,**hist;
  int *W,*region;
  {
  ...
  int mutation_SAMC_prior(WW,fvalue,t,pos,hist)
    double *fvalue, t, **hist;
  int *WW, *pos;
  {
    int i, k, m, region;
    ...
    if(m<=1) add_connection_prior(WW,&fx,t,&region,hist);
    else if(m==connection_threshold) del_connection_prior(WW,&fx,t,&region,hist);
    else{
      if(un<1.0/2) add_connection_prior(WW,&fx,t,&region,hist);
      else  del_connection_prior(WW,&fx,t,&region,hist);
    }
  ...
  */
  ...
  for(iter=1; iter<=(*total_iteration)+warm2; iter++){
    ...
    for(i=1; i<=(*popN); i++){
      ...
      mutation_SAMC_prior(WW[i],&(fvalue[i]),t[i],&(pos[i]),hist);
      
      // if(iter%10000==0) Rprintf("iter=%d pos[%d]=%d fvalue=%g\n",iter,i,pos[i],fvalue[i]);
    }
              ...
              int *total_iteration, int *popN, int *nCPUs)
{
  ...
  int stepscale=20000;
  int WARM=1;
  static double tau=0.6, rho=1.0, delta;
  static double lowE, maxE, bestE, range=50.0, scale=5.0, exL=50.0, exR=0.0, *refden;
  static int grid, extgrid=0, BLOCKSIZE=50;
  static double hightem=1.0, lowtem=1.0, seltem=1.0, EBICgamma=1.0;     
  static double mut_step=0.25;   /* 1.0 */
      static double alpha1=0.2, alpha2=0.2, alpha3=0.2;   /* =1/variance */
      static double prior_var_alpha=0.01, prior_var_beta=0.01;
      static double **data_mat, **test_mat,*wei, **data_y, **test_y;
      static int dim, connection_threshold=25;
      static double accept_mut,accept_change;
      static double total_mut, total_change;
...
#define LOGISTIC(z) 1.0/(1.0+exp(-z))
      ...
      double cost(z,W,aic0,bic0,ebic0)
        double *z,*aic0,*bic0,*ebic0;
      int *W;
      {
      ...
      // begin: "mutationMH.c"
      int Metropolis_mut(z,fz,tem,W,aic,bic,ebic)
        double *z,*fz,tem,*aic,*bic, *ebic;
      int *W;
      {
        ...
        */
        ...
        fnew=cost(y, W,&aicnew,&bicnew,&ebicnew);
        r=-1.0*(fnew-(*fz))/tem;
      ...
      int add_connection(z,fz,tem,W,aic,bic,ebic)
        double *z, *fz, tem, *aic, *bic,*ebic;
      int *W;
      {
        ...
        */
        ...
        if(B==1){
          ...
          fnew=cost(z,W,&aicnew,&bicnew,&ebicnew);
      ...
      int del_connection(z,fz,tem,W,aic,bic,ebic)
        double *z, *fz, tem, *aic,*bic, *ebic;
      int *W;
      {
      ...
      int mutationMH(x,fvalue,t,WW, state,aic,bic,ebic)
        double *x, *fvalue, t,*aic,*bic,*ebic;
      int *WW, state;
      {
        int i, k, m;
        double un, fx, fy, r, aicm, bicm, ebicm;
        ...
        if(state==0) Metropolis_mut(x,&fx,t,WW,&aicm,&bicm, &ebicm);
        else{
          for(m=0, i=1; i<=dim; i++) m+=WW[i];
          
          un=unif_rand();
          if(m<=3){
            if(un<2.0/3) add_connection(x,&fx,t,WW,&aicm,&bicm,&ebicm);
            else Metropolis_mut(x,&fx,t,WW,&aicm,&bicm,&ebicm);
          }
          else if(m==dim){
            if(un<2.0/3) del_connection(x,&fx,t,WW,&aicm,&bicm,&ebicm);
            else Metropolis_mut(x,&fx,t,WW,&aicm,&bicm,&ebicm);
          }
          else{
            if(un<1.0/3) 
              add_connection(x,&fx,t,WW,&aicm,&bicm,&ebicm);
            else if(un<2.0/3) 
              del_connection(x,&fx,t,WW,&aicm,&bicm,&ebicm);
            else Metropolis_mut(x,&fx,t,WW,&aicm,&bicm,&ebicm);
          }
        } 
      ...
      int Metropolis_mut_SAMC(z,fz,tem,W,region,hist,aic,bic,ebic)
        double *z,*fz,tem,**hist,*aic,*bic, *ebic;
      int *W,*region;
      {
        double *y, *d, fnew, r,un,aicnew,bicnew, ebicnew;
      ...
      int add_connection_SAMC(z,fz,tem,W,region,hist,aic,bic,ebic)
        double *z, *fz, tem,**hist,*aic,*bic, *ebic;
      int *W,*region;
      {
        int i,j,m,s,H,k,K,L,d,accept,k1,k2,*D,**node,AA,B;
      ...
      int del_connection_SAMC(z,fz,tem,W,region,hist,aic,bic,ebic)
        double *z, *fz, tem,**hist,*aic,*bic, *ebic;
      int *W,*region;
      {
        int i,j,k,K,m,s,d,H,L,accept,k1,k2,AA,B,*D;
      ...
      int mutation_SAMC(x,fvalue,t,WW,pos,hist,state,aic,bic,ebic)
        double *x, *fvalue, t, **hist,*aic,*bic,*ebic;
      int *WW, *pos,state;
      {
        int i, k, m, region;
        double un, fx, fy, r, aicm, bicm, ebicm;
        ...
        if(state==0) Metropolis_mut_SAMC(x,&fx,t,WW,&region,hist,&aicm,&bicm, &ebicm);
        else{
          for(m=0, i=1; i<=dim; i++) m+=WW[i];
          
          un=unif_rand();
          if(m<=1){
            if(un<2.0/3) add_connection_SAMC(x,&fx,t,WW,&region,hist,&aicm,&bicm, &ebicm);
            else Metropolis_mut_SAMC(x,&fx,t,WW,&region,hist,&aicm,&bicm, &ebicm);
          }
          else if(m==connection_threshold){
            if(un<2.0/3) del_connection_SAMC(x,&fx,t,WW,&region,hist,&aicm,&bicm, &ebicm);
            else Metropolis_mut_SAMC(x,&fx,t,WW,&region,hist,&aicm,&bicm, &ebicm);
          }
          else{
            if(un<1.0/3) 
              add_connection_SAMC(x,&fx,t,WW,&region,hist,&aicm,&bicm, &ebicm);
            else if(un<2.0/3) 
              del_connection_SAMC(x,&fx,t,WW,&region,hist,&aicm,&bicm,&ebicm);
            else Metropolis_mut_SAMC(x,&fx,t,WW,&region,hist,&aicm,&bicm, &ebicm);
          }
        }
...
#pragma omp parallel for private(i,j,iter,k0,m) default(shared)
      for(i=1; i<=inipopN; i++){
        ...
        while(fvalue[i]>1.0e+99){
          ...
          fvalue[i]=cost(x[i],WW[i],&(AIC[i]), &(BIC[i]), &(EBIC[i]));
        }
        
        for(iter=1; iter<=warm0; iter++){
          if(iter<warm0/div) mutationMH(x[i],&(fvalue[i]),1.0,WW[i],0, &(AIC[i]), &(BIC[i]), &(EBIC[i]));
          else mutationMH(x[i],&(fvalue[i]),1.0,WW[i],1, &(AIC[i]), &(BIC[i]), &(EBIC[i])); 
...
#pragma omp parallel for private(i,j,k0,m,iter) default(shared)
        for(i=1; i<=(*popN); i++){ 
          ...
          while(sfvalue[i]>1.0e+99){
            ...
            sfvalue[i]=cost(sx[i],sWW[i],&(AIC[i]), &(BIC[i]), &(EBIC[i]));
          }
          
          for(iter=1; iter<=warm1; iter++){
            if(iter<warm1/div) mutationMH(sx[i],&(sfvalue[i]),t[i],sWW[i],0,&(AIC[i]),&(BIC[i]),&(EBIC[i]));
            else mutationMH(sx[i],&(sfvalue[i]),t[i],sWW[i],1,&(AIC[i]),&(BIC[i]),&(EBIC[i])); 
...
#pragma omp parallel for private(i) default(shared)
        for(i=1; i<=(*popN); i++){
          mutation_SAMC(x[i],&(fvalue[i]),t[i],WW[i],&(pos[i]),hist,1,&(AIC[i]),&(BIC[i]),&(EBIC[i]));
            ...
            if(pos[i]<grid){
              ...
              */ 
              ...
              for(mis1=0, k=1; k<=(*data_num); k++){
                fitting(data_mat[k],x[i],WW[i],ps2);
                for(j=1; j<=(*OUT_UNIT); j++){ 
              ...
              for(mis2=0, k=1; k<=(*test_num); k++){
                fitting(test_mat[k],x[i],WW[i],ps2);
                for(j=1; j<=(*OUT_UNIT); j++){
